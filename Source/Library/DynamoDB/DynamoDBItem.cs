...
namespace AWS.DynamoDB
{
    ...
    public class DynamoDBItem
    {
        ...
        /// <summary>
        /// Gets a dictionary of all the attributes grouped by name
        /// </summary>
        /// <returns>Attributes grouped by name</returns>
        ...
        /// <summary>
        /// Parse an object containing valid "DynamoDB.Attributes" attributes to a "DynamoDBItem"
        /// </summary>
        /// <param name="obj">The object to be parsed</param>
        /// <returns>"DynamoDBItem"</returns>
        public static DynamoDBItem Parse(object obj)
        {
            if (obj == null)
            {
                return null;
            ...
            {
                DynamoDBItem dbditem = new DynamoDBItem();
                Type objType = obj.GetType();
#if (!WINRT && !WINDOWSPHONE)
                var readableProperties = objType.GetProperties().Where(property => property.CanRead);
                var readableHashProperties = readableProperties.Where(property => Attribute.IsDefined(property, typeof(AWS.DynamoDB.Attributes.DynamoDBHashKey)));
                var readableRangeProperties = readableProperties.Where(property => Attribute.IsDefined(property, typeof(AWS.DynamoDB.Attributes.DynamoDBRangeKey)));
                var readableAttributeProperties = readableProperties.Where(property => Attribute.IsDefined(property, typeof(AWS.DynamoDB.Attributes.DynamoDBAttribute)));
#else
                var readableProperties = objType.GetRuntimeProperties().Where(property => property.CanRead);
                var readableHashProperties = readableProperties.Where(property => property.GetCustomAttributes(typeof(AWS.DynamoDB.Attributes.DynamoDBHashKey), false).Count() > 0);
                var readableRangeProperties = readableProperties.Where(property => property.GetCustomAttributes(typeof(AWS.DynamoDB.Attributes.DynamoDBRangeKey), false).Count() > 0);
                var readableAttributeProperties = readableProperties.Where(property => property.GetCustomAttributes(typeof(AWS.DynamoDB.Attributes.DynamoDBAttribute), false).Count() > 0);
#endif
                foreach (PropertyInfo propertyInfo in readableHashProperties)
                {
                    dbditem.Add(propertyInfo.Name, propertyInfo.GetValue(obj, null), isHashKey: true);
                }
                foreach (PropertyInfo propertyInfo in readableRangeProperties)
                {
                    dbditem.Add(propertyInfo.Name, propertyInfo.GetValue(obj, null), isRangeKey: true);
                }
                foreach (PropertyInfo propertyInfo in readableAttributeProperties)
                {
                    dbditem.Add(propertyInfo.Name, propertyInfo.GetValue(obj, null));
                }
                return dbditem;
            }
        }
